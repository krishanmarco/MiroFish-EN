...
Run Twitter and Reddit simulations simultaneously, reading the same config file

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- After simulation completes, environment stays running in command wait mode
- Support receiving Interview commands via IPC
- Support single Agent or batch interviews
...
# ============================================================
# Fix Windows encoding: Set UTF-8 encoding before all imports
# This fixes issues with OASIS third-party library not specifying encoding when reading files
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout to UTF-8 (fix Chinese garbled text in console)
    ...
    # Note: This needs to be set at Python startup, runtime setting may not work
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        # Only set default encoding for text mode (non-binary) without explicit encoding
...
if os.path.exists(_env_file):
    ...
    # Attempting to load backend/.env
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings (we intentionally don't set max_tokens, let model decide)"""
...
# Add filter immediately when module loads to ensure it works before camel code runs
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (records every agent's observation and action), we use our own action_logger
    """
...
# Available Twitter actions (does not include INTERVIEW, which can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (does not include INTERVIEW, which can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    Manage both platform environments, handle Interview commands
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None/not specified: interview both platforms, return combined results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden per interview item)
                - "twitter": only interview Twitter platform
                - "reddit": only interview Reddit platform
                - None/not specified: each agent interviews both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that need to interview both platforms
        ...
        # Split both_platforms_interviews across two platforms
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    Returns:
        Dictionary of agent_id -> entity_name mappings
    """
    agent_names = {}
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from database and supplement full context information

    Args:
        db_path: Database file path
        last_rowid: Last processed rowid value (using rowid instead of created_at because different platforms have different created_at formats)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Quote post: supplement original post content, author, and quote comment
            ...
            # Get quoted post comment content (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Create comment: supplement info about the post being commented on
        ...
        # Context supplement failure does not affect main flow
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prioritize using names in agent_names
            author_name = ''
            if agent_id is not None and agent_id in agent_names:
                author_name = agent_names[agent_id]
            elif user_id:
                # Get names from user table
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, parallel simulations can use different API providers
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key config. Please set LLM_API_KEY in .env file in project root")
...
async def run_twitter_simulation(
    ...
    # If config does not contain an agent, use OASIS default name
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed rowid in database (avoids created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Get actual executed actions from database and record
    ...
    # Note: do not close environment, keep it available for interviews
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation completes, do not enter command wait mode'
    )

    args = parser.parse_args()

    # Create shutdown event at start of main function to ensure entire program can respond to exit signals
    ...
    if args.twitter_only:
        ...
        # Run in parallel (each platform uses its own log recorder)
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation complete! Total elapsed: {total_elapsed:.1f}s")

    # Whether to enter command wait mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command wait mode - environment keeps running")
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep so shutdown_event can be responded to
                try:
    ...
    if twitter_result and twitter_result.env:
        ...
        log_manager.info("[Twitter] environment has been closed")
    
    if reddit_result and reddit_result.env:
        await reddit_result.env.close()
        log_manager.info("[Reddit] environment has been closed")
...
def setup_signal_handlers(loop=None):
    ...
    2. Give program a chance to cleanup resources properly (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (gives loop a chance to cleanup resources)
            if _shutdown_event:
                _shutdown_event.set()

        # Do not call sys.exit() directly, let asyncio loop exit normally and cleanup resources
