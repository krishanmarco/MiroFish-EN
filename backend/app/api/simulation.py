...
# Adding this prefix avoids Agent calling tools and replies directly with text
INTERVIEW_PROMPT_PREFIX = "Based on your character, all past memories and actions, reply directly with text without calling any tools:"


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize interview questions by adding prefix to avoid Agent calling tools
...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by LLM and do not need to be set manually

    Request (JSON):
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "mirofish_xxxx",    // optional, get from project if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - completed: execution complete, preparation was completed long ago
        # - stopped: has stopped, preparation was completed long ago
        # - failed: execution failed (but preparation is complete)
...
def prepare_simulation():
    ...
    This is a time-consuming operation, interface returns task_id immediately.
    ...
    - If preparation is complete, directly return existing results
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // optional, whether to use LLM for character profiles
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "preparation task started|existing preparation work available",
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Existing preparation work available, no need to regenerate",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"Simulation {simulation_id} not prepared yet, will start preparation task")
        ...
        # This way frontend can immediately get expected total Agent count after calling prepare
        try:
            logger.info(f"Synchronously getting entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quick read entities (no need for edge info, just count)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Don't get edge info, for speed
            )
            ...
            # Failure doesn't affect subsequent processes, background task will retry
...
def get_prepare_status():
    ...
    2. Check if completed preparation exists via simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // whether completed preparation exists
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Existing preparation work available",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Get the latest report_id for simulation

    Traverse the reports directory to find reports matching simulation_id.
...
def get_simulation_history():
    ...
    For home page history project display, returns simulation list with rich information
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get files list of associated project (max 3)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Realtime get Agent profiles for simulation (for real-time viewing during generation)

    Differences from /profiles endpoint:
    - Directly read files, bypassing SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (like file modification time, whether generating, etc.)
    ...
    try:
        ...
        logger.error(f"Failed to realtime get profile: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Realtime get simulation config (for real-time viewing during generation)

    Differences from /config endpoint:
    - Directly read files, bypassing SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (like file modification time, whether generating, etc.)
    - Can return partial information even if config generation is not complete
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (includes LLM configuration).

    Returns:
        - time_config: Time configuration (total hours, minutes per round, etc.)
        - agent_configs: Agent configuration (persona, behavior, etc.)
        - event_config: Event configuration (event name, description)
        - platform_configs: Platform configuration
        - generation_reasoning: LLM configuration description
    """
    try:
        manager = SimulationManager()
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Configuration not found. Please run /prepare first."
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Configuration file does not exist. Please run /prepare first."
            }), 404
        ...
        logger.error(f"Download configuration failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def download_simulation_script(script_name: str):
    """
    Download a simulation script file (located in backend/scripts/).
    ...
    """
    try:
        # Locate the backend/scripts/ directory
        ...
        if script_name not in allowed_scripts:
            return jsonify({
                "success": False,
                "error": f"Script not allowed: {script_name}. Options: {allowed_scripts}"
            }), 400
...
def generate_profiles():
    """
    Generate OASIS agent profiles (create persona configuration files).

    Request (JSON):
        {
            "graph_id": "mirofish_xxxx",     // Required: graph ID
            "entity_types": ["Student"],      // Optional: filter by entity types
            "use_llm": true,                  // Optional: use LLM to enrich profiles
            "platform": "reddit"              // Optional: target platform
        }
    """
    try:
        data = request.get_json() or {}
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Missing required field: graph_id"
            }), 400
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // Optional: sync agent activity to Zep graph
            "force": false                         // Optional: force restart even if already running
        }

    force parameter behavior:
        - If true and simulation is already running or completed, stops it and restarts
        - Cleans up: run_state.json, actions.jsonl, simulation.log, and database files
        - Preserves: configuration file (simulation_config.json) and profile files
        - Allows a fresh simulation run

    enable_graph_memory_update behavior:
        - If true, agent actions (posts, likes, comments) are synced to the Zep graph in real time
        - Enables the "memory" layer for AI report generation
        - Requires a project with graph_id configured
        - Updates happen asynchronously and do not block the simulation API
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // Whether a force restart was performed
            }
        }
    """
    try:
        ...
        max_rounds = data.get('max_rounds')  # Optional: round limit
        enable_graph_memory_update = data.get('enable_graph_memory_update', False)  # Optional: graph memory sync
        ...
        # Handle non-READY status (e.g. previously completed, running, paused)
        if state.status != SimulationStatus.READY:
            # Check if the simulation config has already been prepared
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Config exists — simulation was previously prepared or run
                if state.status == SimulationStatus.RUNNING:
                    # Check if the runner process is actually still running
                    run_state = SimulationRunner.get_run_state(simulation_id)
                    if run_state and run_state.runner_status.value == "running":
                        # Simulation is actively running
                        if force:
                            # Force stop the currently running simulation
                            logger.info(f"Force stopping currently running simulation: {simulation_id}")
                            try:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is already running. Use /stop to stop it, or pass force=true to restart."
                            }), 400

                # Clean up previous run logs if force restart requested
                if force:
                    logger.info(f"Cleaning up previous run data for: {simulation_id}")
                    cleanup_result = SimulationRunner.cleanup_simulation_logs(simulation_id)
                    if not cleanup_result.get("success"):
                        logger.warning(f"Cleanup warning: {cleanup_result.get('errors')}")
                    force_restarted = True

                # Simulation is prepared — reset status to READY to allow re-run
                logger.info(f"Simulation {simulation_id} is prepared, resetting status to ready (was: {state.status.value})")
                state.status = SimulationStatus.READY
                manager._save_simulation_state(state)
            else:
                # Configuration is not prepared
                return jsonify({
                    "success": False,
                    "error": f"Simulation is not prepared. Current status: {state.status.value}. Please run /prepare first."
                }), 400
        
        # Resolve graph_id for graph memory update
        graph_id = None
        if enable_graph_memory_update:
            # Try to get graph_id from simulation state first
            graph_id = state.graph_id
            if not graph_id:
                # Fall back to getting graph_id from the project
                project = ProjectManager.get_project(state.project_id)
                if project:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Cannot enable graph memory update: graph_id not found. Ensure the project has a graph built."
                }), 400
...
def get_run_status(simulation_id: str):
    """
    Get simulation running status (for polling)
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed simulation running status (with action feed)
    ...
    try:
        ...
        # Get current round's actions (latest round, for recent_actions display)
        ...
        # recent_actions contains the latest actions from the current round
...
def get_simulation_actions(simulation_id: str):
    """
    Get agent actions in the simulation.
...
def get_simulation_timeline(simulation_id: str):
    ...
    Useful for visualizing simulation progress round by round.

    Query parameters:
        start_round: First round to include (default 0)
        end_round: Last round to include (default: all rounds)

    Returns per-round aggregate information.
    """
    try:
...
def get_agent_stats(simulation_id: str):
    ...
    Useful for analyzing individual agent activity, action counts, and action types.
    """
    try:
...
def get_simulation_posts(simulation_id: str):
    """
    Get posts from the simulation database.

    Query parameters:
        platform: Platform to query (twitter/reddit)
        limit: Maximum results to return (default 50)
        offset: Offset for pagination

    Returns data directly from the SQLite database.
    """
    try:
        platform = request.args.get('platform', 'reddit')
...
def get_simulation_comments(simulation_id: str):
    """
    Get comments from the simulation database (Reddit platform).
...
def interview_agent():
    ...
    Note: Requires simulation to be in running status (completed or pending state is not supported).
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional: specify platform (twitter/reddit)
                                               // If omitted, interviews on all available platforms
            "timeout": 60                      // Optional: timeout in seconds (default 60)
        }

    Returns (no platform specified, dual-platform):
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    try:
        ...
        if not prompt:
            return jsonify({
                "success": False,
                "error": "Missing required field: prompt (interview question)"
            }), 400
        ...
        # Check simulation environment is alive
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running. Ensure the simulation has been started and is in running or pending state."
            }), 400

        # Optimize the prompt to constrain agent response format
        ...
        return jsonify({
            "success": False,
            "error": f"Interview request timed out: {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    Note: Requires simulation to be in running status.

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required: simulation ID
            "interviews": [                    // Required: list of interview requests
                {
                    "agent_id": 0,
                    "prompt": "What is A's view?",
                    "platform": "twitter"      // Optional: override platform for this agent
                },
                {
                    "agent_id": 1,
                    "prompt": "What is B's view?"  // Uses global platform default
                }
            ],
            "platform": "reddit",              // Optional: global platform default (overrides per-item platform)
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify({
                "success": False,
                "error": "Missing required field: interviews (must be a list)"
            }), 400

        # Validate global platform parameter
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": "platform must be 'twitter' or 'reddit'"
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview entry {i+1} platform must be 'twitter' or 'reddit'"
                }), 400
        ...
        # Optimize each prompt to constrain agent response format
        ...
        return jsonify({
            "success": False,
            "error": f"Batch interview request timed out: {str(e)}"
        }), 504
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required: simulation ID
            "prompt": "What do you think?",         // Required: prompt to send to all agents
            "platform": "reddit",                   // Optional: target platform (twitter/reddit)
                                                    // If omitted: interviews all agents on both platforms
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"Interview all agents request timed out: {str(e)}"
        }), 504

    except Exception as e:
        logger.error(f"Interview all agents failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_interview_history():
    ...
    Queries interview records stored in the simulation database.

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",  // Required: simulation ID
            "platform": "reddit",          // Optional: filter by platform (reddit/twitter)
                                           // If omitted, returns history from all platforms
            "agent_id": 0,                 // Optional: filter by specific agent ID
...
def get_env_status():
    ...
    Used to check whether the environment is alive before attempting interviews.
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Environment is running and available for interviews"
            }
        }
    """
    try:
        ...
        if env_alive:
            ...
            message = "Environment is not running"
...
def close_simulation_env():
    ...
    Sends a close signal to the running OASIS environment, allowing it to save state before exiting.
    After closing, the simulation moves to 'pending' status and can be resumed later.

    Note: This is different from /stop. /stop terminates the process immediately.
    /close-env sends a graceful close request so the environment can save its state cleanly.
    ...
    try:
        ...
        logger.error(f"Close simulation environment failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
