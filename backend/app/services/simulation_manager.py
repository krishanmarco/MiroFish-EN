...
Manages parallel simulation on Twitter and Reddit dual platforms
...
class SimulationManager:
    ...
    3. Intelligently generate simulation config parameters using LLM
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from Zep graph
        2. Generate OASIS Agent Profile for each entity (optional LLM enhancement, supports parallelization)
        3. Use LLM to intelligently generate simulation config parameters (time, activity level, posting frequency, etc.)
        4. Save config and profile files
        5. Copy preset scripts to simulation directory

        Args:
            simulation_id: Simulation ID
            simulation_requirement: Simulation requirement description (for LLM to generate config)
        ...
        try:
            ...
            # Reddit is already saved in real-time during generation, save again to ensure completeness
            if progress_callback:
                progress_callback(
            ...
            # ========== Phase 3: LLM-generated simulation config ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0,
            ...
            # Note: Run scripts remain in backend/scripts/ directory and are not copied to simulation directory
            # When starting the simulation, simulation_runner will run scripts from scripts/ directory
