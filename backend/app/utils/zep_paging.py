...
This module wraps automatic pagination logic (including per-page retries)
and transparently returns complete lists to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch single page with exponential backoff retry. Only retries transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Paginate through graph nodes, return at most max_items (default 2000). Each page request includes retry logic."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate through graph edges and return complete list. Each page request includes retry logic."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
