            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation complete, but the entire section may not be finished) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics
    const factMatch = text.match(/Related prediction facts:\s*(\d+)/i)
    const entityMatch = text.match(/Involved entities:\s*(\d+)/i)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract statistics
    const nodesMatch = text.match(/Total number of nodes:\s*(\d+)/i)
    const edgesMatch = text.match(/Total number of edges:\s*(\d+)/i)
    const activeMatch = text.match(/Current valid facts:\s*(\d+)/i)
    const histMatch = text.match(/History\/Expired facts:\s*(\d+)/i)
    ...
    // Extract currently active facts - full extraction, no count limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interviewee count (e.g. "5 / 9 simulation agents")
    const countMatch = text.match(/\*\*Number of interviews:\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### Reason for interview subject selection\n([\s\S]*?)(?=\n---\n|\n### Interview records)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }

    // Parse each individual's selection reason
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract bio
      const bioMatch = block.match(/_Brief introduction:\s*([\s\S]*?)_\n/i)
      if (bioMatch) {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling if the first question is preceded by "1."
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with legacy format: only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer; copy as default display only when it's not placeholder text
          ...
          // No platform markers (very old format), treat the whole text as the answer
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > \u201Ctext\u201D (Chinese curly quotes)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*related/i)
    ...
    // Attempt to extract edge information (if available)
    ...
    // Attempt to extract node information (if available)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether the text is a platform placeholder
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(Platform response)'
        || t === '(No response from Twitter platform)'
        || t === '(No response from Reddit platform)'
        || t === '（该平台未获得回复）'
        || t === '(该平台未获得回复)'
        || t === '[无回复]'
        || t === '[No reply]'
    ...
    // Attempt to split the answer by question numbering
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX：" or "QuestionX:" (Chinese format, new backend format)
      // 2. "1. " or "\n1. " (number + dot, legacy format compatibility)
      let matches = []
      let match

      // Prefer "Question X:" format (English or legacy Chinese)
      const cnPattern = /(?:^|[\r\n]+)(?:Question\s*|问题)(\d+)[：:]\s*/gi
      ...
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^(?:Question\s*|问题)\d+[：:]\s*/i, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
        return [cleaned || answerText]
      }

      // Extract each section by numbering
      const parts = []
    ...
    // Check whether a question has dual-platform answers (filtering placeholder text)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (non-placeholder) and the content differs
...
// Currently active step (used for the top header display)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading level-2 heading (## xxx) since the section title is shown in the outer layer
  ...
  // Remove whitespace after opening list tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Remove whitespace before closing list tags
  ...
  // Remove <p><br> immediately preceding block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Remove consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove <br> before paragraph opening tags that follow block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> elements are separated by paragraph content, keep numbering sequential
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in nextTick after the loop completes
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is complete, scroll to top; otherwise scroll to bottom to follow the latest log
            if (isComplete.value) {
...
// Extract the final answer content - extracts section content from an LLM response
const extractFinalContent = (response) => {
  if (!response) return null

  // Attempt to extract content within <final_answer> tags
  ...
  // Attempt to find content after "Final Answer:" (supports multiple formats)
  ...
  // Attempt to find content after Chinese "最终答案:"
  ...
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the thinking process that starts with "Thought:"
