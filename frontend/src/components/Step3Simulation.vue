...
// Reset all state (used for restarting simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
// Track last round per platform to detect changes and log them
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        ...
        stopPolling()
        // Final detail fetch to ensure all actions are loaded before transitioning
...
const checkPlatformsCompleted = (data) => {
  ...
  // Check all enabled platforms are complete
