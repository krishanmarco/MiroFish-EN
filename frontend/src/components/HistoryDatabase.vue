        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be launched while running and do not support historical replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Compute actual height: rows * card height + (rows-1) * gap + small bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute cards per row to center each row
    ...
    const x = offset * 35
    // Adjusted start position: close to title with appropriate spacing
...
// Get progress class based on round count
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, just record state and handle after animation
        ...
        // Debounce state change to prevent rapid flicker
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs executing (may have been overridden by later scroll)
          ...
          setTimeout(() => {
            ...
            // After animation, check for any new pending state
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink viewport bottom, requires more scrolling to trigger expand
...
.grid-pattern {
  ...
  /* Positioned from top-left; expanding height only grows at bottom, preserving existing grid position */
