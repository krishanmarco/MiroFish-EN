    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing. Consider refreshing the graph manually.</span>
...
const renderGraph = () => {
  ...
  // Process edge data, compute edge count and index between same node pairs
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge - only one merged self-loop per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Stores details of all self-loop edges
        }
      })
      return
    }
    ...
    // Determine if edge direction matches normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // Compute curvature: spread out for multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Ensures all edges are distributed in the same frame of reference
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to draw isolated node clusters toward center
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only, don't restart simulation (differentiate click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if a real drag has started (movement exceeds threshold)
